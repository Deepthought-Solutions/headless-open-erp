...
from infrastructure.web.app import app, get_current_user, get_db
from infrastructure.web.auth import oauth2_scheme, TokenData
from domain.orm import Lead, Contact, Company, Position, Concern, LeadPosition, LeadConcern, LeadUrgency, LeadStatus, NoteReason, Base, RecommendedPack
...
def client():
    ...
    # Seed enum tables
    db.add_all([
        NoteReason(name='appel sortant'), NoteReason(name='mail sortant'),
        NoteReason(name='appel entrant'), NoteReason(name='mail entrant'),
        NoteReason(name='rencontre'), LeadStatus(name='nouveau'),
        LeadStatus(name='à rappeler'), LeadStatus(name='relancé'),
        LeadStatus(name='proposition envoyée'), LeadStatus(name='gagné'),
        LeadStatus(name='perdu'), LeadUrgency(name='immédiat'),
        LeadUrgency(name='ce mois'), LeadUrgency(name='moyen terme'),
        RecommendedPack(name='conformité'), RecommendedPack(name='confiance'),
    ...
    # Tear down the database and clear overrides
    db.close()
...
def seed_db(client):
    ...
    lead = Lead(
        ...
        problem_summary="A test message", status_id=status.id, urgency_id=urgency.id
    )
    db.add(lead)
    db.commit()
...
def get_altcha_payload(client):
    ...
    challenge_dict['max_number'] = challenge_dict.pop('maxNumber')
    ...
    payload = {
        ...
        "number": solution.number, "salt": challenge.salt,
        "signature": challenge.signature, "took": solution.took,
    }
    ...
    with patch('application.notification_service.EmailNotificationService.send_lead_notification_email') as mock_send_email:
        ...
        data = {
            "lead": {
                ...
                "job_title": "CEO", "company_name": "Doe Inc.", "company_size": 10,
                "positions": ["Developer", "Manager"], "concerns": ["PSSI", "Dev"],
                "problem_summary": "This is a test message.", "estimated_users": 5,
                "urgency": "ce mois", "conscent": True
            },
...
def test_create_lead_invalid_email(client):
    ...
    data = {
        "lead": {
            "name": "John Doe", "email": "john.doe", "company_name": "Doe Inc.",
            "positions": [], "concerns": [], "urgency": "ce mois", "conscent": True
        },
        "altcha": altcha_payload
    }
    response = client.post('/lead/', json=data)
...
@pytest.mark.parametrize("note_data, expected_status, expected_response", [
    ({"note": "", "reason": "appel sortant"}, 400, {"error": "Bad Request", "message": "The following fields are required: note", "missing_fields": ["note"]}),
    ({"note": "This is a test note.", "reason": ""}, 400, {"error": "Bad Request", "message": "The following fields are required: reason", "missing_fields": ["reason"]}),
    ({"note": "", "reason": ""}, 400, {"error": "Bad Request", "message": "The following fields are required: note, reason", "missing_fields": ["note", "reason"]}),
    ({"note": "This is a test note.", "reason": "invalid reason"}, 400, {"detail": "Reason 'invalid reason' not found"})
])
...
def test_create_and_get_notes(client, seed_db):
    ...
    lead = db.query(Lead).first()
    db.close()
    note_data = {"note": "This is a test note.", "reason": "appel sortant"}
    response = client.post(f"/leads/{lead.id}/notes", json=note_data)
    assert response.status_code == 200
...
def test_list_leads_unauthenticated(client):
    app.dependency_overrides = {} # Clear overrides for this specific test
    response = client.get("/leads/")
    assert response.status_code == 401
    # No need to restore overrides, as the fixture will handle it for the next test
...
def test_create_lead_missing_data(client):
    altcha_payload = get_altcha_payload(client)
    data = {"lead": {"name": "John Doe", "email": "john.doe@example.com"}, "altcha": altcha_payload}
    response = client.post('/lead/', json=data)
    assert response.status_code == 422
